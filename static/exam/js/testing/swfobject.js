$(function () {
    ...
    var appWidth = 24;
    var appHeight = 24;
    var flashvars = {'upload_image': '../../images/icon_record_upload.png'};
    var params = {};
    ...
    $(document).on('click',".recorder .start-recording",function() {
        ...
        currentObj = $(this).parents(".questionsContent").length>0?$(this).parents(".questionsContent"):$(this).parents(".question-content");
    ...
    // remove recorde
    $(document).on('click',".audio-list .glyphicon-remove",function(e) {
        e.stopPropagation();
        e.preventDefault();
        currentObj = $(this).parents(".questionsContent").length>0?$(this).parents(".questionsContent"):$(this).parents(".question-content");
        var question_id=currentObj.attr("questionsId");
            ...
            success:function(msg) {
                if(msg.msg=="true"){
                    ...
                    $(".questionsContent").each(function(index, element) {
                        if($(this).attr("questionsId")==question_id){
                            var audioLength=$(this).find("dd").children(".audio-list").children().length;
                            if(audioLength == 0){
                                $("#numberCardDom a.questions_"+question_id).removeClass("icon_answer_answered").addClass("icon_answer_no");
                                $("#numberCardDom a.questions_"+question_id).parent("dd").removeClass("s2").addClass("s1");
                                $("#numberCardDom a.questions_"+question_id).parent(".s1").removeClass("s2").addClass("s1");
        ...
        switch (arguments[0]) {
            ...
            case "saved":
                ...
                if(msg.msg=='true'){
                    ...
                    $("#numberCardDom a.questions_"+questionsId).parent(".s1").removeClass("s1").addClass("s2");
                    $(".questionsContent").each(function(index, element) {
                    ...
                    $(".audio-list .glyphicon-remove").click(function(e) {
                        e.stopPropagation();
                        e.preventDefault();
                        var question_id=$(this).parents(".questions-content").attr("questionsId");
                        var pobj=$(this).parent(".audio-row");
                        var audio_url=$(pobj).find("audio").attr("src");
                        $.ajax({
                            type: "POST",
